import { apiUrl, COLORS, STORAGE_KEYS } from "@/constants";
...
import { handleReqError } from "@/services";
import { FileType } from "@/services/types";

export interface OkkDataType {
  resident_id: number;
...
export const updateOkkData = async (
  ...
  try {
    const localData =
      (await storageService.getData(STORAGE_KEYS.okkData)) || {};
    //@ts-ignore
    await storageService.setData(STORAGE_KEYS.okkData, {
      ...localData,
      [status]: data,
    });
  } catch (e) {}
...
const getResFiles = (res: OkkDataType[]) => {
  const files: string[] = [];
  try {
    res.map((item) => {
...
export const getOkkTasks = async (
  ...
  try {
    ...
    if (res?.data) {
      const edited: OkkDataType[] = res.data.map((item) => ({
        ...item,
      ...
      await updateOkkData(params.okk_status_code, edited || []);
    ...
    if (e?.code === "ERR_NETWORK" || e?.code === "ECONNABORTED") {
      const localData = await storageService.getData(STORAGE_KEYS.okkData);
...
export const sendOkkCheck = async (
  help_call_id: number,
  ...
  try {
    ...
    return await handleReqError(e, args, "sendOkkCheck");
  }
};
